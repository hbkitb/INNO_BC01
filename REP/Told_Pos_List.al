report 50103 "Told Pos ERPG"
{
    DefaultLayout = RDLC;
    WordLayout = 'REP/ToldPOS.docx';
    Caption = 'Told Pos';
        ...
        dataitem("Sales Invoice Header"; "Sales Invoice Header")
        {
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    column(Consignor_Kolli; "Sales Invoice Header"."Consignor Kolli")
                    {
                    ...
                    column(BilltoCustNo_SalesInvHdrCaption; SalesInvBillToCustLbl)
                    //FIELDCAPTION("Bill-to Customer No."))
                    {
                    }
                    column(PricesInclVAT_SalesInvHdrCaption; SalesInvPricesIncVatLbl)
                    ...
                    dataitem("Sales Invoice Line"; "Sales Invoice Line")
                    {
                        ...
                        begin
                            ...
                            Clear(ToldPos);
                            ToldPos.SetRange(id, "Sales Invoice Header"."No.");
                            ToldPos.SetRange(Code, item."Inventory Posting Group");

                            if not ToldPos.FindFirst() then begin
                                ToldPos.ID := '';
                                ToldPos.Init();

                                ToldPos.Quantity := ToldPos.Quantity + Quantity;
                                ToldPos.Amount := ToldPos.Amount + "Line Amount";
                                ToldPos."Net Weight" := ToldPos."Net Weight" + "Net Weight";
                                ToldPos.Weight := ToldPos.Weight + "Gross Weight";
                                ToldPos.Code := Item."Inventory Posting Group";
                                ToldPos.ID := "Sales Invoice Header"."No.";
                                ToldPos.Description := Item."Inventory Posting Group";

                                if InventoryPostingGroup.Get(Item."Inventory Posting Group") then
                                    ToldPos.ToldPos := InventoryPostingGroup.ToldPos;
                    ...
                    dataitem(ToldPos_; "ToldPos ERPG")
                    {
                        //WHERE (Number = CONST (1));
                        // WHERE (Number = FILTER (1 ..));
                        ...
                        begin
                            GrosWightTotal += Weight;
                            NetWightTotal += "Net Weight";
                            PosAmountTotal += Amount;
                        end;
                        ...
                        begin
                            //hbk170221ToldPos_.FilterGroup(2);
                            //hbk170221ToldPos_.SetRange(id, "Sales Invoice Header"."No.");

                        end;

                    }
                    dataitem(ToldTotal; Integer)
                    {
                        DataItemTableView = SORTING(Number)
                                            WHERE(Number = CONST(1));

                        column(Gros_Wight_Total; GrosWightTotal) { }
                        column(Net_Wight_Total; NetWightTotal) { }
                        column(Pos_Amount_Total; PosAmountTotal) { }
                ...
                begin
                    ...
                    ToldPos.SetRange(id, "Sales Invoice Header"."No.");
                    if ToldPos.FindSet() then
                        repeat
                            ToldPos.Amount := 0;
                            ToldPos."Net Weight" := 0;
                            ToldPos.Weight := 0;
                            ToldPos.Quantity := 0;
                            ToldPos.Modify();
                        until ToldPos.Next() = 0;
            ...
            var
                Handled: Boolean;
            begin
                if SalesSetup.NoComp <> "Sales Invoice Header"."Bill-to Customer No." then
                    CurrReport.Quit();

                //CurrReport.LANGUAGE := Language.GetLanguageID("Language Code");
                CurrReport.LANGUAGE := ReportLanguage.GetLanguageID("Language Code"); //HBK / ITB - erstatter ovenst. - 030120
    ...
    var
        ...
        SalesInvBillToCustLbl: Label 'Bill To Customer';
        SalesInvPricesIncVatLbl: Label 'Prices incl. vat';
        SalesInvOrderNoLbl: Label 'Order No';
        SalesInvYourRefLbl: Label 'Your Ref';
        SalesInvVatRegNoLbl: Label 'Vat Reg. No.';
        ...
        ToldPos: Record "ToldPos ERPG";
        Item: Record Item;
        GrosWightTotal: Decimal;
        NetWightTotal: Decimal;
        PosAmountTotal: Decimal;
        InventoryPostingGroup: Record "Inventory Posting Group";
        "Country/Region": Record "Country/Region";
        ReportLanguage: Codeunit Language;  //HBK/ITB-030120
    ...
    begin
        ...
        //Caption_ := FIELDCAPTION(SalesInvoiceHeader."Order No.");
        OrderNoText := FormatDocument.SetText(SalesInvoiceHeader."Order No." <> '', SalesInvOrderNoLbl);
        //FIELDCAPTION(SalesInvoiceHeader."Order No."));
        ReferenceText := FormatDocument.SetText(SalesInvoiceHeader."Your Reference" <> '', SalesInvYourRefLbl);
        //FIELDCAPTION(SalesInvoiceHeader."Your Reference"));
        VATNoText := FormatDocument.SetText(SalesInvoiceHeader."VAT Registration No." <> '', SalesInvVatRegNoLbl);
        //FIELDCAPTION(SalesInvoiceHeader."VAT Registration No."));
        //END;
    end;
    ...
    begin
        ...
        //ValueEntry.SETRANGE("Document Type", "Sales Invoice Line"."Document Type"::"Sales Invoice");
    ...
    begin
        ...
        END ELSE BEGIN
            //  LineFeeNoteOnReportHist.SETRANGE("Language Code", Language.GetUserLanguage); //Removed in Version 15
            LineFeeNoteOnReportHist.SETRANGE("Language Code", 'DK'); //Removed in Version 15
    ...
    [IntegrationEvent(false, false)]  //HBK / ITB - 030120
    [Scope('Cloud')]
